...
class BaseRunner(ABC):
    ...
    #     target_test_size = 20
    #     if len(test_dataset) > target_test_size:
    #         print(f"DEBUG MODE: Cắt giảm test dataset từ {len(test_dataset)} xuống {target_test_size} ảnh.")
    #         test_dataset = torch.utils.data.Subset(test_dataset, range(target_test_size))
    ...
    #                                  shuffle=False,
    #                                  num_workers=1,
    #                                  drop_last=False, # Lưu ý: drop_last=True có thể làm mất ảnh nếu batch_size > số ảnh còn lại
    #                                  sampler=test_sampler)
    #     else:
    #         test_loader = DataLoader(test_dataset,
    #                                  batch_size=self.config.data.test.batch_size,
    #                                  shuffle=False,
    #                                  num_workers=1,
    #                                  drop_last=False) # Nên sửa thành False để đảm bảo chạy đủ 20 ảnh
