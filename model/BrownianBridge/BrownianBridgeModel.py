...
class BrownianBridgeModel(nn.Module):
    ...
    def forward(self, x, y, context=None):
        if self.condition_key == "nocond":
            context = None
        # RLI-DM: context (Xương cá) được truyền từ bên ngoài
    ...
    def p_losses(self, x0, y, context, t, noise=None):
        ...
        unet_input = torch.cat([x_t, y], dim=1) 
        objective_recon = self.denoise_fn(unet_input, timesteps=t, context=context)
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            unet_input = torch.cat([x_t, y], dim=1) # 6 kênh input
            objective_recon = self.denoise_fn(unet_input, timesteps=t, context=context)
            x0_recon = self.predict_x0_from_objective(x_t, y, t, objective_recon=objective_recon)
            if clip_denoised: x0_recon.clamp_(-1., 1.)
